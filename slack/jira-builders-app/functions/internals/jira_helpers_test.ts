...
import { parseJiraIdsFromString } from "./jira_helpers.ts";

Deno.test("Single jira ticket in simple string", async () => {
    const jiras = parseJiraIdsFromString("Verification thread for FOO-1234");
    assertEquals(jiras.length, 1);
    assertEquals(jiras[0], "FOO-1234");
});

Deno.test("Multiple jira ticket in simple string", async () => {
    const jiras = parseJiraIdsFromString("Verification thread for FOO-1234 and LOL-909");
